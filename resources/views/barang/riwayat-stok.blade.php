...
<x-ui.card class="filter-card mb-4"><form method="GET" action="/barang/{{ $barang->id }}/riwayat-stok" class="filter-grid"><div><label class="form-label" for="supplier_id">Supplier Asal</label><select class="form-select" id="supplier_id" name="supplier_id"><option value="">Semua Supplier</option>@foreach($supplier_options as $supplier)<option value="{{ $supplier->id }}" @selected((string) ($filters['supplier_id'] ?? '') === (string) $supplier->id)>{{ $supplier->nama_supplier }}{{ (!$supplier->is_active || $supplier->trashed()) ? ' (tidak aktif)' : '' }}</option>@endforeach</select></div><div><label class="form-label" for="warehouse_id">Gudang</label><select class="form-select" id="warehouse_id" name="warehouse_id"><option value="">Semua Gudang</option>@foreach($warehouse_options as $warehouse)<option value="{{ $warehouse->id }}" @selected((string) ($filters['warehouse_id'] ?? '') === (string) $warehouse->id)>{{ $warehouse->kode_gudang }} · {{ $warehouse->nama_gudang }}{{ (!$warehouse->is_active || $warehouse->trashed()) ? ' (tidak aktif)' : '' }}</option>@endforeach</select></div><div class="filter-actions"><x-ui.button type="submit" icon="ti-search">Terapkan</x-ui.button><x-ui.button href="/barang/{{ $barang->id }}/riwayat-stok" variant="outline-secondary" icon="ti-reload">Reset</x-ui.button></div></form></x-ui.card>
