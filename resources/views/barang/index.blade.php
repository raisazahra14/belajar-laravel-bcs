...
<x-ui.card class="filter-card mb-4">
    <form action="{{ route('barang.index') }}" method="GET" class="filter-grid inventory-filter-grid" id="inventory-filter-form" data-results-endpoint="{{ route('barang.results') }}">
        <div class="filter-search"><label class="form-label" for="search">Pencarian</label><div class="input-icon"><i class="ti-search"></i><input class="form-control" id="search" type="search" name="search" placeholder="Kode, nama, atau rak/lokasi detail" value="{{ request('search') }}"></div></div>
        ...
        <div><label class="form-label" for="supplier_id">Supplier</label><select class="form-select" id="supplier_id" name="supplier_id"><option value="">Semua Supplier</option>@foreach($supplier_options as $supplier)<option value="{{ $supplier->id }}" @selected((string) request('supplier_id') === (string) $supplier->id)>{{ $supplier->nama_supplier }}{{ (!$supplier->is_active || $supplier->trashed()) ? ' (tidak aktif)' : '' }}</option>@endforeach</select></div>
        <div><label class="form-label" for="warehouse_id">Gudang</label><select class="form-select" id="warehouse_id" name="warehouse_id"><option value="">Semua Gudang</option>@foreach($warehouse_options as $warehouse)<option value="{{ $warehouse->id }}" @selected((string) request('warehouse_id') === (string) $warehouse->id)>{{ $warehouse->kode_gudang }} · {{ $warehouse->nama_gudang }}{{ (!$warehouse->is_active || $warehouse->trashed()) ? ' (tidak aktif)' : '' }}</option>@endforeach</select></div>
