...
@section('content')
<div class="page-header"><div><h1>Detail Barang</h1><p>Informasi lengkap persediaan barang.</p></div><div class="page-actions"><x-ui.button :href="route('barang.index')" variant="outline-secondary" icon="ti-arrow-left">Kembali</x-ui.button><x-ui.button href="/barang/{{ $barang->id }}/riwayat-stok" variant="outline-primary" icon="ti-time">Riwayat Stok</x-ui.button>@can('update-stock')<x-ui.button href="/barang/{{ $barang->id }}/stok" icon="ti-pencil-alt">Update Stok</x-ui.button>@endcan</div></div>
...
<div class="col-md-6 mb-4"><p class="detail-label">Stok Saat Ini</p><p class="detail-value"><x-ui.badge :variant="$barang->stok <= 5 ? 'danger' : 'success'">{{ number_format($barang->stok, 0, ',', '.') }} {{ $barang->satuan }}</x-ui.badge></p></div>
